...
UPSTREAM_REPO=${UPSTREAM_REPO:-${UPSTREAM_REPO:-ntorga/agent-starter-kit}}
UPSTREAM_BRANCH=${UPSTREAM_BRANCH:-${UPSTREAM_BRANCH:-main}}
TARGET_BRANCH=${TARGET_BRANCH:-${TARGET_BRANCH:-main}}
MERGE_STRATEGY=${MERGE_STRATEGY:-${MERGE_STRATEGY:-merge}}
ALLOW_UNRELATED=${ALLOW_UNRELATED:-${ALLOW_UNRELATED:-false}}
PUSH=${PUSH:-${PUSH:-true}}
RESOLUTION_MODE=${RESOLUTION_MODE:-${RESOLUTION_MODE:-attempt}}
...
if [ "${MERGE_STRATEGY}" = "rebase" ]; then
  ...
  # default merge (no-ff)
  MERGE_CMD=(git merge --no-ff upstream/${UPSTREAM_BRANCH} -m "chore: sync from upstream/${UPSTREAM_BRANCH}")
  ...
  if ! "${MERGE_CMD[@]}"; then
    ...
    prefer_local_patterns=("AGENTS.md" "rules/**" "skills/**" "personas/**")
    prefer_upstream_patterns=()
    ...
    python skills/assets/check_markdown.py .context.md README.md AGENTS.md CHANGELOG.md docs/.context.md docs/CONTRIBUTING.md docs/features.md docs/roadmap.md || true
