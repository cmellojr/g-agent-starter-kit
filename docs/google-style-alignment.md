...
Plan to align the Agent Starter Kit's coding conventions, review practices, and
tooling with [Google Style Guides][styleguide] and [Google Engineering
Practices][eng-practices].

## Guiding Decisions

| Decision | Choice |
|---|---|
| Git workflow | Migrate from Conventional Commits to [How to Write a Git Commit Message][cbea-git] (Google style) |
...
| Rust style | Official [Rust Style Guide][rust-style] (Google does not maintain one) |
...
|---|---|---|
| `rules/edicts/code-style-python.md` | [Google Python Style Guide][py-style] | Line length 80, 4 spaces, `snake_case`, docstrings, `import x`, type annotations, exceptions, simple comprehensions |
| `rules/edicts/code-style-go.md` | [Google Go Style Guide][go-style] | `gofmt`, MixedCaps, no fixed line length, Least mechanism, clarity > concision |
| `rules/edicts/code-style-shell.md` | [Google Shell Style Guide][shell-style] | Bash `#!/bin/bash`, 2 spaces, 80 cols, `local`, `main()`, `$(...)`, `[[...]]`, no `eval`, ShellCheck |
| `rules/edicts/code-style-rust.md` | [Rust Style Guide][rust-style] + [API Guidelines][rust-api] | `rustfmt`, 4 spaces, 100 cols, naming conventions, `Result`, traits |
| `rules/edicts/code-style-markdown.md` | [Google Markdown Style Guide][md-style] | 80 cols, ATX headings, fenced code blocks with language, `[TOC]`, reference links, no trailing whitespace |
...
- **Keep:** KISS, DRY, SRP, Data Trust Boundary, Error Handling (must be
  logged), Process-killing exceptions
- **Remove:** sections now covered by language-specific guides (naming, imports,
  testing structure)
...
- Replace with [How to Write a Git Commit Message][cbea-git] rules:
...
3. **Design pass** (NEW) — appropriateness, simplicity, maintainability, API
   design
...
Checklist based on [Google Code Review — What to Look For][what-to-look-for]:
...
- Step 1: load **project rules + Google Style Guide** for the detected
  language
- Detect language by file extension to select the correct guide
- Naming violations: reference the specific guide (`snake_case` for Python,
  `MixedCaps` for Go)

### 4.5 Update `skills/reviewer-self-review.md` (SHIELD)

- Expand **S** (Scan All Passes Complete) to include **Design pass** and
  **Documentation pass**
...
- Step 4a: "Absorb local coding style **and consult the Google Style Guide for
  the language**"
- Reference new `rules/edicts/code-style-*` explicitly in the playbook

### 5.2 Update `skills/coder-self-review.md` (GRASP)

- **S (Style)** score 2 now requires: "follows Google Style Guide conventions
  for the language"
- **G (Guidelines)**: add docstring verification per applicable guide

### 5.3 Update `skills/dispatch.md`

- Step 6: detect language to include the correct Google Style Guide in
  `<rules>` block
- Example: task touches `.py` files → include
  `rules/edicts/code-style-python.md`
...
- Add scopes: `coding-python`, `coding-go`, `coding-shell`, `coding-rust`,
  `coding-markdown`
...
- Add note: Constraints/Guidance sections may reference the applicable Google
  Style Guide
...
- Step 6 (Standards checklist): verify plan against Google Style Guide for the
  language
...
- **Google Testing Best Practices** — [Testing on the Toilet][tot-blog] series
  + structured testing guidance
- **Google Engineering Practices** — deeper integration with [How to Do a Code
...
[cbea-git]: https://cbea.ms/git-commit/
[rust-style]: https://doc.rust-lang.org/style-guide/
[rust-api]: https://rust-lang.github.io/api-guidelines/
[py-style]: https://google.github.io/styleguide/pyguide.html
[go-style]: https://google.github.io/styleguide/go/guide
[shell-style]: https://google.github.io/styleguide/shellguide.html
[md-style]: https://google.github.io/styleguide/docguide/style.html
[what-to-look-for]: https://google.github.io/eng-practices/review/reviewer/
[tot-blog]: https://testing.googleblog.com/
[how-to-review]: https://google.github.io/eng-practices/review/reviewer/
